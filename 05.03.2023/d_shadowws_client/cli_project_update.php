...
if($_SERVER["REQUEST_METHOD"] == "GET") {
  $user_id = $_GET['user_id'];
   $sql = "SELECT update_title,update_date,updater_name,project_id   FROM project_update where client=$user_id";
   $result = mysqli_query($db, $sql);
 if ($result) {
  $rows = array();
  while ($row = mysqli_fetch_assoc($result)) {
$project_id = $row['project_id'];
 $project_name_query = "SELECT name FROM project WHERE id=$project_id";
